...
@section ('main1')
 ...
 {{-- --}}
 {{-- <p><a href="{{ route('register') }}?redirect_url={{ route('courses.show', [$course->slug]) }} "class="btn btn-primary">Buy course (${{ $course->price }})</a></p> --}}
    ...
    <p>
        @if (\Auth::check())
            ...
            <form action="{{ route('coursePayment') }}" method="POST">
 ...
 @foreach ($lessons->where('published',1) as $lesson)
    @if ($lesson->free_lesson)(FREE!) @endif{{$loop->iteration}}.
        <a href="{{route('lessons.show',[$lesson->course->first()->id, $lesson->slug])}}">{{$lesson->title}}</a>
