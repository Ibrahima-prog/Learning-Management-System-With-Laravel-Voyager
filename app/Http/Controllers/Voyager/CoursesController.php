...
class CoursesController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
      ...
      //  dd($dataType->browseRows[9]->details->model::get());
...
//dd(TeachersCourse::whereIn('user_id',[Auth::user()->role_id])->get());
//$tests=DB::table('teachers_courses')->whereIn('user_id',[Auth::user()->id])->get()->pluck("course_id");
//dd($test);
//dd($dataType->model_name);
if (strlen($dataType->model_name) != 0) {
    $model = app($dataType->model_name);//$model=$model->find(2);
//dd($model->scope);
    if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
        $query = $model->{$dataType->scope}();
    } else {
       $query = $model::select('*');//first store the foreach of the pivot table for comparison,pivot table find it
       // dd($query);
      // echo $query;
    }
    ...
    if ($orderBy && in_array($orderBy, $dataType->fields())) {
       ...
       $dataTypeContent1 = call_user_func([
        ...
        $dataTypeContent0 = call_user_func([$query->latest($model::CREATED_AT), $getter]);

        $dataTypeContent1 = $dataTypeContent0;
    } else {
       $dataTypeContent1 = call_user_func([$query->orderBy($model->getKeyName(), 'DESC'), $getter]);
    }
   // dd($model);

//dd( call_user_func([$query->latest($model::CREATED_AT), $getter]));
    // Replace relationships' keys for labels and create READ links if a slug is provided.
    $dataTypeContent1 = $this->resolveRelations($dataTypeContent1, $dataType);
} else {
    // If Model doesn't exist, get data from table name
    $dataTypeContent1 = call_user_func([DB::table($dataType->name), $getter]);
    $model = false;
}
...
// Eagerload Relations
$this->eagerLoadRelations($dataTypeContent1, $dataType, 'browse', $isModelTranslatable);
...
// Actions
//dd($dataTypeContent1->where('id','4'));
$actions = [];
if (!empty($dataTypeContent1->first())) {
    foreach (Voyager::actions() as $action) {
        $action = new $action($dataType, $dataTypeContent1->first());
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function create(Request $request)
    {
        ...
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        //dd(app($dataType->addRows[7]->details->model)->whereNotIn('role_id',['1','3'])->get());
       // $model = app($dataType->model_name);
       // dd($model::OfTeachers()->get());
        // Check permission
  ...
  //  dd($dataType->addRows[8]->details->model::find(3));
  //dd($dataType->addRows[8]->details->model::whereNotIn('id',['1'])->pluck('id','name'));
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
