...
class QuestionsController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $question= Question::find($dataTypeContent->id)->options()->get();
       ...
       // $options2=$options;
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','old'));
    }
    ...
    public function update(Request $request, $id)
    {
      ...
      //  dd($request);
        ...
        //dd($data->id);
        //$questionId=$try->id;
        $question= Question::find($data->id)->options()->get();
        foreach ($question as $q) {
            $previous[]=$q;
          //$old[]=$q->option_text;
            # code...
         ...
         for ($i=0; $i <4 ; $i++) {
            ...
            // dd($previous[$i]->id);
            $insertOption=QuestionsOption::where('id',$previous[$i]->id)->first();
            //$insertPivot=new PivotQuestionsOption();
            $hh="me".$i;$bb="checkme".$i;
             //dd($request->$hh);
            $insertOption->option_text=$request->$hh;

            if ($request->$bb=='on') {
             $insertOption->correct=1;
            }
            else{
                $insertOption->correct=0;
            }
            $insertOption->save();
    ...
    {
//dd($request);
//$insertOption=new QuestionsOption();$insertPivot=new PivotQuestionsOption();
        $slug = $this->getSlug($request);
        ...
        $data = $this->insertUpdateData($request, $slug, $dataType->addRows, $try=new $dataType->model_name());
        $questionId=$try->id;
         for ($i=0; $i <4 ; $i++) {
            $insertOption=new QuestionsOption();$insertPivot=new PivotQuestionsOption();
            $hh="me".$i;$bb="checkme".$i;
            ...
            $insertOption->save();
            $optionId=$insertOption->id;
            $insertPivot->question_id=$questionId;
            $insertPivot->questions_option_id=$optionId;
            $insertPivot->save();
         }
