...
class LessonsController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
     ...
     //dd($courselesson->where('lesson_id',3)->get()->pluck('course_id'));
       // dd(course::active()->get()->pluck('id'));
       //$CourseId= $courselesson->whereIn('course_id',course::active()->get()->pluck('id'))->get()->pluck('course_id');
      // dd($tests=$courselesson->whereIn('course_id',$CourseId)->get()->pluck('lesson_id'));
//dd($request->input("course_id"));
   if ($request->input("course_id")) {
        # code...
        $courseid=$request->input("course_id");

$lessonid=$courselesson->where('course_id',$courseid)->get()->pluck("lesson_id");
 //$lessons->whereIn("id",$lessonid);
//dd($request->input("course_id"));
   }
   else{
       $lessonid=$lessons->all()->pluck("id");
   }
        //dd($courses->where('id',2)->get());
       ...
       // dd($request);
        // Check permission
     // dd( (app($dataType->model_name)->course($request))->getModel());
        ...
        if (strlen($dataType->model_name) != 0) {
            $model = app($dataType->model_name);//dd($model);
            //dd($query = $model->{$dataType->scope}());
            if ($request->input("course_id")) {
                # code...
                $query = $model::whereIn("id",$lessonid);
            }
            elseif ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope' . ucfirst($dataType->scope))) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            ...
            $redirect = redirect()->route("voyager.{$dataType->slug}.index",["course_id"=>$request->lesson_belongstomany_course_relationship[0]]);
        } else {
    ...
    public function create(Request $request)
    {
        ...
        $slug = $this->getSlug($request);
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
       // dd(app($dataType->addRows[9]->details->model));
        $method = $request->input('method', 'add');
//dd($dataType->dataTypeRows);
    //$pos = (Lesson::max('position')+1);
        // Check permission
...
//dd($request);
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
               // dd($request->lesson_belongstomany_course_relationship[0]);
              // dd(route("voyager.{$dataType->slug}.index"));
              // dd($dataType->slug);
                $redirect = redirect()->route("voyager.{$dataType->slug}.index",["course_id"=>$request->lesson_belongstomany_course_relationship[0]]);
            } else {
                $redirect = redirect()->back();
